// Package testsuite contains reusable behavioral tests for Faneuil Hall implementations.
package testsuite
...
func Run(t *testing.T, newImpl func() FaneuilHall) {
	t.Helper()
	...
	t.Run("CancelWaitingEntrant", func(t *testing.T) {
		for _, tc := range []struct {
			name string
			run  func(fh FaneuilHall, ctx context.Context) <-chan error
		}{
			{
				name: "Immigrant",
				run: func(fh FaneuilHall, ctx context.Context) <-chan error {
...
func Benchmark(b *testing.B, newImpl func() FaneuilHall) {
	b.Helper()
	b.ReportAllocs()
