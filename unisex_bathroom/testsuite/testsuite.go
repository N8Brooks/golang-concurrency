// Package testsuite contains reusable behavioral tests for unisex bathroom
// implementations.
package testsuite
...
func startMale(b UnisexBathroom, ctx context.Context) worker {
	w := worker{
...
func startFemale(b UnisexBathroom, ctx context.Context) worker {
	w := worker{
		entered: make(chan struct{}),
		release: make(chan struct{}),
		done:    make(chan error, 1),
...
func runCapacityWave(t *testing.T, b UnisexBathroom, ctx context.Context, male bool) {
	t.Helper()
...
func Run(t *testing.T, newImpl func() UnisexBathroom) {
	t.Helper()
	...
	t.Run("CancelWhileWaitingForOppositeSex", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			nextMale := startMale(b, t.Context())
			synctest.Wait()
			requireClosed(t, nextMale.entered, "male did not enter after canceled waiter was removed")

			close(nextMale.release)
			synctest.Wait()
			requireSuccess(t, nextMale.done, "male did not complete")
		})
	})
	...
	t.Run("CancelWhileWaitingForCapacity", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			requireClosed(t, replacement.entered, "replacement male did not enter after canceled waiter freed the slot")
...
func Benchmark(b *testing.B, newImpl func() UnisexBathroom) {
	b.Helper()
	b.ReportAllocs()
