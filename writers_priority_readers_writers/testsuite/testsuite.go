// Package testsuite contains reusable behavioral tests for writers-priority
...
type WritersPriorityReadersWriters interface {
...
func startReader(rw WritersPriorityReadersWriters, ctx context.Context) worker {
	w := worker{
...
func startWriter(rw WritersPriorityReadersWriters, ctx context.Context) worker {
	w := worker{
		entered: make(chan struct{}),
		release: make(chan struct{}),
		done:    make(chan error, 1),
...
func runReadersShare(t *testing.T, rw WritersPriorityReadersWriters, ctx context.Context) {
	t.Helper()
...
func runWriterExclusive(t *testing.T, rw WritersPriorityReadersWriters, ctx context.Context) {
	t.Helper()
...
func Run(t *testing.T, newImpl func() WritersPriorityReadersWriters) {
	t.Helper()
	...
	t.Run("QueuedWritersBlockLaterReadersUntilAllWritersFinish", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			rw := newImpl()
			ctx := t.Context()
			...
			writer1 := startWriter(rw, ctx)
			synctest.Wait()
			requireOpen(t, writer1.entered, "first writer entered while reader was active")

			writer2 := startWriter(rw, ctx)
			synctest.Wait()
			requireOpen(t, writer2.entered, "second writer entered while reader was active")

			lateReader := startReader(rw, ctx)
			synctest.Wait()
			requireOpen(t, lateReader.entered, "late reader bypassed queued writers")

			close(activeReader.release)
			synctest.Wait()

			requireSuccess(t, activeReader.done, "active reader did not complete")
			requireClosed(t, writer1.entered, "first queued writer did not enter after readers left")
			requireOpen(t, writer2.entered, "second queued writer entered before first writer finished")
			requireOpen(t, lateReader.entered, "late reader entered before first writer finished")

			close(writer1.release)
			synctest.Wait()

			requireSuccess(t, writer1.done, "first queued writer did not complete")
			requireClosed(t, writer2.entered, "second queued writer did not enter after first writer finished")
			requireOpen(t, lateReader.entered, "late reader entered while another writer remained queued")

			close(writer2.release)
			synctest.Wait()

			requireSuccess(t, writer2.done, "second queued writer did not complete")
			requireClosed(t, lateReader.entered, "late reader did not enter after all queued writers finished")
...
func Benchmark(b *testing.B, newImpl func() WritersPriorityReadersWriters) {
	b.Helper()
	b.ReportAllocs()
