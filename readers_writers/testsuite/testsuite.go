// Package testsuite contains reusable behavioral tests for readers-writers implementations.
package testsuite
...
func startReader(rw ReadersWriters, ctx context.Context) worker {
	w := worker{
...
func startWriter(rw ReadersWriters, ctx context.Context) worker {
	w := worker{
		entered: make(chan struct{}),
		release: make(chan struct{}),
		done:    make(chan error, 1),
...
func runReadersShare(t *testing.T, rw ReadersWriters, ctx context.Context) {
	t.Helper()
...
func runWriterExclusive(t *testing.T, rw ReadersWriters, ctx context.Context) {
	t.Helper()
...
func Run(t *testing.T, newImpl func() ReadersWriters) {
	t.Helper()
	...
	t.Run("CanceledReaderDoesNotBlockFutureWriter", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			rw := newImpl()
			...
			next := startWriter(rw, t.Context())
			synctest.Wait()
			requireClosed(t, next.entered, "future writer did not enter after canceled reader was removed")

			close(next.release)
			synctest.Wait()
			requireSuccess(t, next.done, "future writer did not complete")
		})
	})
}

func Benchmark(b *testing.B, newImpl func() ReadersWriters) {
	b.Helper()
	b.ReportAllocs()
