// Package testsuite contains reusable behavioral tests for extended child-care
// implementations.
package testsuite
...
type ExtendedChildCare interface {
...
func startChild(c ExtendedChildCare, ctx context.Context) worker {
	w := worker{
...
func startAdult(c ExtendedChildCare, ctx context.Context) worker {
	w := worker{
		entered: make(chan struct{}),
		release: make(chan struct{}),
		done:    make(chan error, 1),
...
func runCapacityWave(t *testing.T, c ExtendedChildCare, ctx context.Context) {
	t.Helper()
...
func Run(t *testing.T, newImpl func() ExtendedChildCare) {
	t.Helper()
	...
	t.Run("CompetingAdultsLeaveSafely", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			c := newImpl()
			ctx := t.Context()
	...
	t.Run("WaitingAdultDoesNotBlockLegalChildren", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			c := newImpl()
			ctx := t.Context()
			...
			close(adults[0].release)
			synctest.Wait()
			requirePending(t, adults[0].done, "adult left even though four children still needed two adults")

			children[4] = startChild(c, ctx)
			synctest.Wait()
			requireClosed(t, children[4].entered, "child did not enter while an adult was waiting to leave")
			requirePending(t, adults[0].done, "adult left immediately after another child entered")

			close(children[0].release)
			synctest.Wait()
			requireSuccess(t, children[0].done, "first child did not complete")
			requirePending(t, adults[0].done, "adult left after only one child exited from five")

			close(children[1].release)
			synctest.Wait()
			requireSuccess(t, children[1].done, "second child did not complete")
			requireSuccess(t, adults[0].done, "adult did not complete once the child count dropped back to three")
			...
			requireSuccess(t, adults[1].done, "second adult did not complete")
		})
	})
...
func Benchmark(b *testing.B, newImpl func() ExtendedChildCare) {
	b.Helper()
	b.ReportAllocs()
